...
{
    SCE_btstart ();
    SCE_CDeleteShaderCG (shader->v);
    SCE_CDeleteShaderCG (shader->p);
    SCE_btend ();
}
#endif
...
#ifdef SCE_USE_CG
    SCE_SHADER_FOR (".cg", SCE_CG_SHADER, SCE_UNKNOWN_SHADER)
    SCE_SHADER_FOR (".vcg", SCE_CG_SHADER, SCE_VERTEX_SHADER)
    SCE_SHADER_FOR (".pcg", SCE_CG_SHADER, SCE_PIXEL_SHADER)
    SCE_SHADER_FOR (".fcg", SCE_CG_SHADER, SCE_PIXEL_SHADER)
    SCE_SHADER_FOR (".cgvs", SCE_CG_SHADER, SCE_VERTEX_SHADER)
...
{\
    ...
    {\
        ...
        SCE_CSetShader##bign##Source (shader->v, shader->vs_source);\
...
\
    ...
    {\
        ...
        SCE_CSetShader##bign##Source (shader->p, shader->ps_source);\
...
(GLSL, 
 shader->p_glsl = SCE_CCreateProgram ();
 if (!shader->p_glsl)
 {
     SCE_CDeleteShaderGLSL (shader->v);
     SCE_CDeleteShaderGLSL (shader->p);
 ...
 if (shader->v)
     SCE_CSetProgramShader (shader->p_glsl, shader->v, 1);
 if (shader->p)
     SCE_CSetProgramShader (shader->p_glsl, shader->p, 1);
 if (SCE_CBuildProgram (shader->p_glsl) < 0)
 {
     SCE_CDeleteProgram (shader->p_glsl);
     SCE_CDeleteShaderGLSL (shader->v);
     SCE_CDeleteShaderGLSL (shader->p);
...
static int SCE_Shader_GetIndexCG (SCE_SShader *s, int t, const char *n)
{
    if (t == SCE_PIXEL_SHADER)
        return (int)SCE_CGetShaderCGIndex (s->p, n);
    else
        return (int)SCE_CGetShaderCGIndex (s->v, n);
}
#endif
static int SCE_Shader_GetIndexGLSL (SCE_SShader *s, int t, const char *n)
{
    (void)t;
    return SCE_CGetProgramIndex (s->p_glsl, n);
}
...
{
    return SCE_CGetProgramAttribIndex (s->p_glsl, n);
}
...
{
    SCE_CSetShaderCGParam ((CGparameter)SCE_Shader_GetIndexCG (used, t, n), v);
}
#endif
static void SCE_Shader_ParamGLSL (int t, const char *n, int v)
{
    SCE_CSetProgramParam (SCE_CGetProgramIndex (used->p_glsl, n), v);
}
...
{
    SCE_CSetShaderCGParamf ((CGparameter)SCE_Shader_GetIndexCG (used, t, n), v);
}
#endif
static void SCE_Shader_ParamfGLSL (int t, const char *n, float v)
{
    SCE_CSetProgramParamf (SCE_CGetProgramIndex (used->p_glsl, n), v);
}
...
{\
    SCE_CSetShaderCGParam##n##fv ((CGparameter)SCE_Shader_GetIndexCG\
                                  (used, t, na), s, v);\
}
...
static void SCE_Shader_Param##n##fvGLSL (int t, const char *na,\
                                         size_t s, float *v)\
{\
    SCE_CSetProgramParam##n##fv (SCE_CGetProgramIndex (used->p_glsl, na), s, v);\
}\
...
{
    SCE_CSetShaderCGParam ((CGparameter)i, v);
}
#endif
static void SCE_Shader_SetParamGLSL (int i, int v)
{
    SCE_CSetProgramParam (i, v);
}
...
{
    SCE_CSetShaderCGParamf ((CGparameter)i, v);
}
#endif
static void SCE_Shader_SetParamfGLSL (int i, float v)
{
    SCE_CSetProgramParamf (i, v);
}
...
#ifdef SCE_USE_CG
SCE_CG_SETPARAM_FUNC(1)
SCE_CG_SETPARAM_FUNC(2)
SCE_CG_SETPARAM_FUNC(3)
SCE_CG_SETPARAM_FUNC(4)
#endif
...
{
    SCE_CSetProgramMatrix3 (i, 1, m); /* count forced */
}
...
{
    SCE_CSetShaderCGMatrix ((CGparameter)i, m);
}
#endif
static void SCE_Shader_SetMatrix4GLSL (int i, SCE_TMatrix4 m)
{
    SCE_CSetProgramMatrix4 (i, 1, m); /* count forced */
}
...
static void SCE_Shader_UseCG (SCE_SShader *shader)
{
    ...
    {
        if (shader->v)
            SCE_CUseShaderCG (shader->v);
        if (shader->p)
            SCE_CUseShaderCG (shader->p);
    }
    else
        SCE_CUseShaderCG (NULL);
}
#endif
...
static void SCE_Shader_UseGLSL (SCE_SShader *shader)
{
    if (!sce_shd_enabled)
        ...
        SCE_CUseProgram (shader->p_glsl);
    else
        SCE_CUseProgram (NULL);
}
